                        ...
                        <div class="row my-3 mx-2">
                            <div class="col-md-2 column__right___padding">
                                ...
                                <input type="text" class="form-control form-control-sm" name="code" id="code"
                                    v-model="search_user.code" placeholder="Code" @keyup="getSearch" />
                            </div>
                            <div class="col-md-2 column__right___padding column__left___padding">
                                ...
                                <input type="text" class="form-control form-control-sm" name="name" id="name"
                                    v-model="search_user.name" placeholder="Vendor's Name" @keyup="getSearch" />
                            </div>
                            ...
                            <div class="table-responsive">
                                <div class="d-flex flex-row mb-3 rounded">
                                    <div class="left d-flex">
                                        ...
                                        <div class="p-2 border icon_item" >
                                            <a @click.prevent="activeSelectedItems(checkUserItems)">
                                                <font-awesome-icon class="icon_item-icon" icon="fa-solid fa-circle-check"
                                                    color="#0bd018" />
                                            </a>
                                        </div>
                                        <div class="p-2 border icon_item" >
                                            <a @click.prevent="inactiveSelectedItems(checkUserItems)">
                                                <font-awesome-icon class="icon_item-icon" icon="fa-solid fa-circle-minus"
                                                    color="#eb0505" />
                                            </a>
                                        </div>
                                        <div class="p-2 border icon_item" v-if="this.checkUserItems.length >0">
                                            <a href="javascript:void(0)"
                                                @click.prevent="deleteSelectedItems(checkUserItems)">
                                ...
                                <table class="table">
                                        ...
                                        <tr>
                                            <th class="checkArea">
                                                <div class="form-check mb-4">
                                                    ...
                                                    v-if="this.users.length > 0" :checked="this.checkAllItems.length==this.checkUserItems.length"  v-model="checkAllItems" />
                                                </div>
                                            </th>
                                    ...
                                    <tbody>
                                        <tr v-for="user in users" :key="user.id" :class="rowClass">
                                            <td class="checkArea">
                                                <div class="form-check mb-4">
                                                    <input class="form-check-input" type="checkbox"
                                                        v-model="checkUserItems" v-bind:value="user"
                                                        v-bind:id="user.id" />
                                                </div>
                                            </td>
                                            <td :class="iconClassBody" class="pt-2">
                                                <label v-if="user.status == 1"
                                                    class="badge bg-success text-white fw-bold">Active</label>
                                                <label v-if="user.status == 0"
                                            ...
                                            <td :class="textClassBody" >
                                                <a href="javascript:void(0)" @click.prevent="editUser(user.id)">
                                        ...
                                        <div class="row mb-1">
                                            ...
                                            <div class="col-md-9">
                                                <input type="text" class="form-control form-control-sm" name="code"
                                                    id="code" v-model="user.code" placeholder="Code" required />
                                            </div>
...
export default {
    ...
    watch: {
        ...
        checkAllItems(value) {
            ...
            if (this.checkUserItems.length == this.users.length) {
                this.checkUserItems = [];
            } else {
                this.checkUserItems = this.users;
            }
        },
    ...
    methods: {
            ...
            const tableData = (
                await axios.get(route("users.all"), {
                    params: {
                        ...
                        search_vendor_code: this.search_user.code,
                        search_vendor_name: this.search_user.name,
                    }
                })
            ).data;
        ...
        async getVendors() {
            ...
            const users = (await axios.get(route("users.all"))).data;
        ...
        async createUser() {
            ...
            try {
                if (this.select_country) {
                    this.user.country_id = this.select_country.id;
                }
                if (this.select_currency) {
                    this.user.currency_id = this.select_currency.id;
                }
                const user = (await axios.post(route("users.store"), this.user))
                    .data;
        ...
        async editUser(userId) {
            ...
            window.location.href = route("users.edit", userId);
        },
        async clearFilters() {
        ...
        async inactiveSelectedItems(checkUserItems) {
            this.$root.loader.start();
            const ids = this.checkUserItems.map((vendor) => vendor.id);
            axios
                .post(route("users.inactive.selected'"), { ids })
                .then((response) => {
                    this.checkUserItems = [];
                    this.reload();
                });
        ...
        async activeSelectedItems(checkUserItems) {
            this.$root.loader.start();
            const ids = this.checkUserItems.map((vendor) => vendor.id);
            axios.post(route("users.active.selected"), { ids }).then((response) => {
                this.checkUserItems = [];
                this.reload();
            });
            this.$root.loader.finish();
        ...
        selectAll: function (event) {
            if (event.target.checked == false) {
                ...
                this.vendors.forEach((vendor) => {
                    this.checkUserItems.push(vendor.id);
                });
            }
        },

        async deleteSelectedItems(checkUserItems) {
            this.$root.loader.start();
            try {
                Swal.fire({
                    title: "Are you sure?",
                    ...
                    if (result.isConfirmed) {
                        const ids = this.checkVendorItems.map((user) => user.id);
                        axios.post(route("users.delete.selected"), { ids })
                            .then((response) => {
                                this.reload();
                            });
                        // Swal.fire(
