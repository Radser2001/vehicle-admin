...
export default {
  ...
  methods: {
    ...
    async reload() {
      ...
      const tableData = (
        await axios.get(route("vehicles.bank.all", this.vehicle_id), {
          params: {
            page: this.page,
    ...
    async getBanks() {
      ...
      const banks = (await axios.get(route("vehicles.bank.all", this.vehicle_id)))
        .data;
    ...
    async updateBankData() {
      ...
      try {
        await axios.post(route("vehicles.bank.update", this.vehicle_id), this.bank);
        this.bank = {};
        this.reload();
      ...
      try {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            axios.delete(route("vehicles.bank.delete", id)).then((response) => {
              this.reload();
            });
          }
        });
