<template>
    <div aria-live="assertive"
        ...
        <transition-group tag="div" class="w-full items-center space-y-4 sm:items-end" name="slide-fade">
            <div v-for="notification in notifications" :key="notification.key"
                ...
                <div class="p-3">
                    <div class="flex items-start">
                        <div class="flex-shrink-0">
                            ...
                            <i class="fa-solid fa-triangle-exclamation fa-3x text-danger mt-1"
                                v-else-if="notification.type == 'error'"></i>
                        </div>
                        ...
                        <div class="ml-4 flex-shrink-0 flex">
                            <i class="fa-solid fa-square-xmark fa-2x text-danger cursor-pointer"
...
onMounted(() => {
    disposerTimer.value = setInterval(() => {
        notifications.value = notifications.value.filter(
            (notification) => notification.timeout > new Date().getTime()
        );
    }, 10);
});
...
function success({ title = "Succuss", message, timeout = 4000 }) {
    notifications.push({
        key: Math.random(),
        title,
        message,
